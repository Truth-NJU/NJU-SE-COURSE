...
//        if ((unsigned char)(rootEntry_ptr->DIR_Name[0])==0xe5){
//            fliter = true;
//        }
    ...
    while (value < 0xFF8) {
        ...
        while (index < 512) {
            ...
            if ((tempContent[index + 11] & 0x10) == 0) {
                bool fliter = false;
                for (int h = index; h < index + 11; h++) {
                    if (!(((tempContent[h] >= 48) && (tempContent[h] <= 57)) ||
                          ((tempContent[h] >= 65) && (tempContent[h] <= 90)) ||
                          ((tempContent[h] >= 97) && (tempContent[h] <= 122)) ||
                          (tempContent[h] == ' '))) {
                        fliter = true;    //非英文及数字、空格
                        break;
                    }
    ...
    while (true) {
        ...
        if (command == "ls") {
            ...
            if (!fileOrdirectory(filename)) {
                cout << "不是一个普通文件，无法读取" << endl;
            } else {
                getFileContentThroughClus("../b.img", filename);
            }
            ...
            if (!correct) cout << "参数错误" << endl;
                ...
                for (int i = 0; i < commandSplit.size(); i++) {
                    if (commandSplit[i][0] == '/') {
                        ...
                        if (fileOrdirectory(name)) cout << "请输入正确的目录名称" << endl;
