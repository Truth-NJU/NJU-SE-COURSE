...
base_path = "D:/NJU/21/数据科学基础/大作业/searchindex"
...
def getBaiduIndex(path):
    ...
    for key in keywords:
        ...
        index_df = gp.baidu_search_index(word = key, start_date = '2020-01-01', end_date = '2020-12-31', cookie = cookie)
        ...
        for i in range(days):
            index_dictory[str(date[i]).split()[0][5:7]] = index_dictory.get(str(date[i]).split()[0][5:7], 0) + index[i][0]
        with open(filepath, 'a+') as f:
            for month in index_dictory:
                f.write(month + "|" + str(index_dictory[month]) + "\n")
        print(key + " finished!!")
