...
public class ClockView extends View {
    ...
    private void drawClockPoint(Canvas canvas) {
        ...
        canvas.drawLine(getWidth() / 2, getHeight() / 2, getWidth() / 2, getHeight() / 2 - 130, mPaint);//时针长度设置为130

        //回复canvas的原始状态
        canvas.restore();
        canvas.save();
        canvas.rotate(minAngle,getWidth() / 2, getHeight() / 2);
        canvas.drawLine(getWidth() / 2, getHeight() / 2, getWidth() / 2, getHeight() / 2 - 180 , mPaint);//分针长度设置为180

        canvas.restore();
        canvas.save();
        canvas.rotate(secAngle,getWidth() / 2, getHeight() / 2);
        canvas.drawLine(getWidth() / 2, getHeight() / 2, getWidth() / 2, getHeight() / 2 - 220 , mPaint);//秒针长度设置为220
    ...
    private void drawClockScale(Canvas canvas) {
        ...
        //计算12点处的结束坐标
        int stopX = startX;
        int stopY1 = startY - 50;//整点处的线长度为50
        int stopY2 = startY - 20;//非整点处的线长度为20
        //计算画布每次旋转的角度 6度
        float degree = 360 / 60;
        // 画60根刻度线
        for(int i = 0; i < 60; i++){
            if(i % 5 == 0)
                //绘制整点长的刻度
                canvas.drawLine(startX, startY, stopX, stopY1, mPaint);
            else
                //绘制非整点处短的刻度
                canvas.drawLine(startX, startY, stopX, stopY2, mPaint);
            //以圆中心进行旋转
