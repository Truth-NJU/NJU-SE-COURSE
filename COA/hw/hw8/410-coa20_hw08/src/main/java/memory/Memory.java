...
public class Memory implements MemoryInterface{
    ...
    public void setPAGE(boolean a){
        this.PAGE=a;
    }

    public void setSEGMENT(boolean a){
        this.SEGMENT=a;
    }
    ...
    public String seg_load(int segIndex) {
        ...
        //diskBase是段在磁盘中的物理地址，可以用来读取磁盘中的数据
        ...
        // code here
        while(occupiedSize()+len>MEM_SIZE_B) {
            removeSegByLRU();
        }
    ...
    public String page_load(int segIndex, int pageNO) {
        ...
        //建议加载半个tlb容量的页
        int pages = TLB.TLB_SIZE_B / (TLB.LINE_SIZE_B * 2);
        int count = 0;
        //通过页号从磁盘中加载数据，页号✖️页大小就是当前页的起始地址，读取数据的大小为页的大小
        char[] data = Disk.getDisk().read(t.intToBinary(String.valueOf(pageNO * PAGE_SIZE_B)), PAGE_SIZE_B);

        int base = Integer.parseInt(t.binaryToInt(String.valueOf(getBaseOfSegDes(segIndex))));
        int limit = Integer.parseInt(t.binaryToInt(String.valueOf(getLimitOfSegDes(segIndex))));
        //起始页号
        ...
        //循环读取页的内容
        while (count<=pages) {
            //得到没有映射到虚页的页框，如果页框全部映射到了虚页，就使用removePageByLRU来获取一个有效页
            int flag = reversedPageTbl.length + 1;
            for (int i = startFrame; i <= endFrame; i++) {
                //如果反向页表有页框没映射到虚页中，则该页框可用
                if (!reversedPageTbl(i).isValid) {
                    flag = i;
                    break;
                }
            ...
            int validPage;
            //代表没有页框可用，需要自己产生一个有效页（页框可以看做物理页号）
            if (flag == reversedPageTbl.length + 1) {
                validPage = removePageByLRU(startFrame, endFrame);
            } else validPage = flag;

            //由物理页号得到真正的地址
            String realFrame = t.intToBinary(validPage * PAGE_SIZE_B + "");
            //第一页的物理地址为返回值
            if (count == pages - 1) res = realFrame;

            write(realFrame, PAGE_SIZE_B, data);

            //设置页表和反向页表
            PageItem pageItem = pageTbl(pageNO);
            pageItem.frameAddr = realFrame.toCharArray();
            pageItem.isInMem = true;
            ReversedPageItem reversedPageItem = reversedPageTbl(validPage);
            reversedPageItem.isValid = true;
            reversedPageItem.vPageNO = pageNO;
            reversedPageItem.updateTimeStamp();

            //向tlb中写入数据，第一个参数为虚拟页号，第二个参数为物理页号
            tlb.writeTLB(int2chars(pageNO), realFrame.toCharArray());
    ...
    private int removePageByLRU(int startFrame, int endFrame) {
        ...
        int pageNo = reversedPageTbl(minIndex).vPageNO;
        if(tlb.isMatch( pageNo )!=-1) tlb.invalid( tlb.isMatch( pageNo ) );
        return minIndex;
    }
...
//        System.out.println("Start Fit:" + occupiedSegs.size());
    ...
    public int chars2int(char[] chars){
