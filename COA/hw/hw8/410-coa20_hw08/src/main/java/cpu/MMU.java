...
public class MMU {
	...
	public char[] read(String logicAddr, int length) {
		...
		if (!Memory.PAGE) {  // 实模式或分段模式
			if(!Memory.SEGMENT){
				...
				linearAddr=memory.seg_load(segIndex);
			}
			...
			String virtualPageNum=logicAddr.substring(16,36);
			String pageOffset=logicAddr.substring(36);
			int startvPageNo = Integer.parseInt(t.binaryToInt("0"+virtualPageNum));
			int offset = Integer.parseInt(t.binaryToInt("0"+pageOffset));
			// int endvPageNo = startvPageNo;
			// if(offset!=0){
			// 	endvPageNo=endvPageNo+(length-offset+1)/Memory.PAGE_SIZE_B+1;
			// }else{
			// 	endvPageNo=endvPageNo+(length-offset+1)/Memory.PAGE_SIZE_B;
			// }
	...
	private int getSegIndex(String logicAddr) {
		//TODO
		String segSelector = logicAddr.substring(0, 13);
		return Integer.parseInt(t.binaryToInt("0" + segSelector));
	}
	...
	private String toRealLinearAddr(String logicAddr){
		//TODO
		String offset=logicAddr.substring(32);
		String segSelector=logicAddr.substring(0,16);
		return add((segSelector+"0000").toCharArray(),"0000"+offset);
	}
	...
	private String toSegLinearAddr(String logicAddr) {
		...
		//得到段选择子
		String segSelector=logicAddr.substring(0,13);
		char[] segBase=memory.getBaseOfSegDes(Integer.parseInt(t.binaryToInt(segSelector)));
		return add(segBase,"0000"+offset);
	}
